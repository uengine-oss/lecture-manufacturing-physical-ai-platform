"""S07 · 조치 도구의 실행 전 정책 검사 (학생 시작본 — 동작하지만 검사 세 곳이 틀렸다).
...
def execute_sim_action(repo, sim, event_id: str, approval_id: str | None, action_id: str, value: float, attempt: int = 1, **llm_claims) -> dict:
    ...
    # TODO(학생) 1: 승인 기록이 없을 때 LLM 인자 approved=True 를 승인으로 받아 주고 있다.
    if apr is None and llm_claims.get("approved") is True:
        apr = {"event_id": event_id, "decision": "APPROVED", "action_id": action_id, "approved_value": value}
    if apr is None:
    ...
    # TODO(학생) 2: 요청값이 사람이 승인한 값(approved_value)과 같은지 확인하지 않는다 → VALUE_MISMATCH

    act = repo.allowed_action(ev["asset_id"], ev["event_type"], action_id)
    if act is None:
        reject("ACTION_NOT_ALLOWED", action_id)
    # TODO(학생) 3: 상한만 본다. 승인값 0.4 처럼 하한 아래 값이 통과한다 → VALUE_OUT_OF_RANGE
    if act.get("min_value") is not None and value > act["max_value"]:
