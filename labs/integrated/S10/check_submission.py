...
MAX_SUCCEEDED_ACTIONS = ____  # 같은 사건에서 성공한 조치의 최대 개수 (hydops/config.py max_action_attempts)
APPROVED = "____"             # 실행을 허락하는 승인 결정값
RECOVERED = "____"            # 회복으로 인정하는 재측정 판정값
CLOSED = "____"               # 재측정 회복 후의 사건 상태
CLOSER = "____"               # CLOSED 로 바꿀 수 있는 유일한 행위자 (event_history.actor)
...
def rule_approval(d: dict) -> dict:
    ...
    problems = []
    for a in d["actions"]:
        apr = approvals.get(a["____"])          # 실행 기록이 가리키는 승인 ID 필드
        if apr is None:
        ...
        if not _t(apr["decided_at"]) <= _t(a["____"]):   # 실행 시각 필드
...
def rule_duplicate(d: dict) -> dict:
    keys = [a["____"] for a in d["actions"]]          # 중복 방지 키 필드
...
def rule_verification(d: dict) -> dict:
    ...
    good = [
        ...
        if v["outcome"] == RECOVERED and v["action_log_id"] in action_ts and _t(v["____"]) >= _t(action_ts[v["action_log_id"]])  # 재측정 창 시작 필드
    ]
    problems = []
    if len(good) < ____:
