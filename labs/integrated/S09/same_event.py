...
def same_event_checks(event_id: str, detail: dict, instance: dict | None) -> dict:
    ...
    return {
        "화면 event_id": detail["event"]["____"] == event_id,
        "프로세스 입력 event_id": instance is not None and instance["____"]["event_id"] == event_id,
        # 조치 기록이 없으면(실행 전 종료) None — 실패가 아니라 해당 없음
        "조치 기록 event_id": all(a["____"] == event_id for a in actions) if actions else ____,
    }


def instance_id_of(detail: dict) -> str | None:
    """사건에 대응된 프로세스 인스턴스 ID (event.process_ref)"""
    ref = detail["event"].get("____")
