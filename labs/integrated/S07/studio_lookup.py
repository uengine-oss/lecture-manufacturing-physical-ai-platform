...
PLATFORM_API = "____"                    # 교육용 플랫폼 주소
HYDOPS_API = "http://localhost:8800"     # 로컬 관제 API 주소 (대조용)
SCHEMA_NAME = "____"                     # 수업용 스키마 이름
NOT_PUBLISHED_STATUS = ____              # 발행 전 객체 조회가 돌려주는 HTTP 상태 코드 (S12 화면)


def related_url(cls: str, key: str, rel: str) -> str:
    """관계 조회 주소: GET /studio/schemas/{스키마}/objects/{클래스}/{키}/related/{관계}"""
    return f"{PLATFORM_API}/studio/schemas/{SCHEMA_NAME}/objects/{cls}/{key}/____/{rel}"
...
def classify(result: dict) -> str:
    ...
    if result.get("http_status") != 200:
        return "ERROR"
    if result.get("____", 0) == 0:          # 몇 개가 조회됐는지 담긴 필드 이름
...
def same_sensors(local: dict, platform_related: dict) -> bool:
    ...
    local_ids = sorted(s["____"] for s in local["sensors"])
    platform_ids = sorted(o["____"] for o in platform_related["objects"])
