"""S06 · 이벤트 → 상태 조회 → SOP 검색 → 제안 검사 연결 (정답본).
...
def run_lab_agent(ev: dict, tools: dict) -> dict:
    ...
    win = tools["get_recent_window"](asset_id=a, seconds=60)
    ts = win["sensors"].get("TS1", {})
    if et == "SENSOR_FAULT" or ts.get("sensor_state") == "SENSOR_FAULT":
        sop = tools["search_sop"](asset_id=a, event_type="SENSOR_FAULT")
        cits = [cite(h) for h in sop["hits"][:2]]
        if not cits:
            return proposal("HOLD", rationale="센서 오류 SOP 근거가 없어 보류한다.")
        return proposal("SENSOR_CHECK", "SENSOR_CHECK", None, cits, "센서 품질 불량으로 설비 조치를 보류하고 센서 점검을 제안한다.")

    # ② 대상 설비의 적용 SOP·허용 조치 → ③ 그 설비·사건 유형의 SOP 절 검색
    ctx = tools["get_asset_context"](asset_id=a)
    sop = tools["search_sop"](asset_id=a, event_type=et)
    ...
    if not sop["hits"] or not equipment:
        return proposal("HOLD", rationale="적용 SOP 근거나 허용 조치가 없어 보류한다.")

    # ④ 허용 조치의 기본값으로 제안 검사 — 실행하지 않는다
    act = equipment[0]
    cits = [cite(h) for h in sop["hits"][:3]]
    chk = tools["propose_action"](event_id=ev["event_id"], action_id=act["action_id"], value=act["default_value"], citations=cits)
    if not chk["ok"]:
