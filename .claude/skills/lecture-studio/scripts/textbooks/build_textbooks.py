...
import os
L = Path(os.environ.get("LECTURE_ROOT", ".")).resolve()
TB = L / "textbooks"
DIST = TB / "dist"
FONT = "Pretendard"
...
def main():
    ...
    entries = []
    for cls in sorted(p.name for p in TB.iterdir() if p.is_dir() and p.name not in ("dist",) and not p.name.startswith(("_", "."))):
    ...
    if a.pdf:
        subprocess.run(["node", str(Path(__file__).resolve().parents[1] / "explainer" / "print_pdf.mjs"), str(DIST)], check=True)
