...
def main():
    ...
    # 코덱과 음량만 보면 인코딩이 깨진 파일도 통과한다.
    # (실제로 h264 NAL 이 깨진 영상을 전달한 적이 있다 — 원본 webm 은 멀쩡했다.)
    dec = subprocess.run(
        ["ffmpeg", "-v", "error", "-i", args.out, "-f", "null", "-"],
        capture_output=True, text=True).stderr
    decode_errors = len([l for l in dec.splitlines() if l.strip()])
    print(f"디코드 오류: {decode_errors}줄")

    ok = has_h264 and has_aac and mean_db > -40 and decode_errors == 0
    print("\n결과:", "PASS" if ok else "CHECK FAILED")
    if decode_errors:
        print("  인코딩이 손상됐다. 같은 명령으로 다시 인코딩하면 대개 해결된다.")
