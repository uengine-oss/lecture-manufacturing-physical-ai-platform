...
ASSEMBLE = "/Users/uengine/process-gpt/.claude/skills/process-gpt-demo/scripts/assemble_narrated_video.py"
...
def main():
    ...
    raw = W / "raw" / f"hydops-raw{sfx}.webm"
    ...
    for s in sorted(starts):
        ...
        for x in sents:
            ...
            k += 1
            t += d
    (OUT / f"HydOps_강의시스템_설명영상{sfx}.srt").write_text("\n".join(srt))
    ...
    final = OUT / f"HydOps_강의시스템_설명영상{sfx}.mp4"
    subprocess.run(["ffmpeg", "-y", "-loglevel", "error", "-i", str(mid), "-i", str(OUT / f"HydOps_강의시스템_설명영상{sfx}.srt"), "-map", "0", "-map", "1", "-c", "copy", "-c:s", "mov_text", "-metadata:s:s:0", "language=kor", str(final)], check=True)
