// 실습 영상의 실제 화면 조작 — render.mjs 의 app 장면이 부른다.
...
export async function runApp(page, sc, ctx) {
  ...
  const pr = sc.params || {};
  const release = ['dashboard', 'console', 'platform_flow', 'app_page'].includes(sc.action) ? await lock(path.join(LECTURE, 'video/lab_videos/build/.app.lock')) : () => {};
  ...
  try {
    switch (sc.action) {
      ...
      case 'notebook': {
        const out = path.join(LECTURE, 'video/lab_videos/build/_nb');
